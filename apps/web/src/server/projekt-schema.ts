/**
 * Schema des Projektartefakts (Arbeitsstand), nicht der Offerte — keine Formel.
 *
 * Beschreibt einen veraenderlichen Zwischenstand und laesst deshalb Unvollstaendigkeit zu
 * (ein neues Projekt hat weder Einheiten noch Bewertungen). Die Vollstaendigkeitsregeln der
 * Berechnung greifen erst bei der Projektion (`projektion.ts`).
 *
 * Aggregatregeln stehen in `superRefine`, weil eine Kollision eine Aussage ueber die Menge
...
const bereichsregelSchema = z.object({
  ...
  // `pruefeBereiche` ist die einzige Stelle mit den Staffelinvarianten (Kern,
  // bereichsregel.ts). `normalisiereBereiche` bringt die Zod-Optionalitaet auf die Domainform.
...
// Eigener Name fuer das reine Objektschema: Sobald `refine` angehaengt ist, wird daraus ein
// `ZodEffects`, auf dem `.omit()`/`.pick()`/`.partial()`/`.extend()` nicht mehr existieren.
...
const einheitSchema = z.object({
  ...
  flaecheAussen: z.number().nonnegative(),
  // Bewusst KEIN `stockwerk`: Was eine Einheit vom Referenzobjekt unterscheidet, steht
  // vollstaendig in `spaltenwerte`. `.strict()` weist ein Artefakt mit dem frueheren Feld ab.
...
export const projektSchema = z.object({
  ...
  // Merkmal des ganzen Neubau-Projekts, nicht je Referenzobjekt — optional, damit bereits
  // abgelegte Projekte ohne dieses Feld gueltig bleiben (I-24).
  ...
  // Vom Vermarkter gesetzter Aufwandindikator D: die Anwesenheit entscheidet, nicht die
  // Groesse (Muster `wirksamer-wert.ts`) — fehlt das Feld, gilt der hergeleitete Wert.
  aufwandindikatorUebersteuerung: z.number().min(0).max(1).optional(),
  /**
   * Ebene 2 des Zwei-Ebenen-Modells: projektbezogene Abweichungen von den Firmenwerten,
   * nur die tatsaechlich uebersteuerten Pfade (Delta). Optionalitaet haelt abgelegte
   * Bestandsprojekte gueltig (I-24).
   *
   * Bewusst nur `record(unknown)` und kein Nachbau der Konfigurationsform: Welche Pfade
   * uebersteuerbar sind und welche Werte zulaessig, entscheidet allein `mergeKonfiguration`
   * samt Nachvalidierung im Ladepfad (PE-01). Ein `.strict()`-Schema hier waere eine zweite
   * Wahrheit darueber.
   */
